    ...
    <div class="main-card mb-3 card">
        ...
        <div class="table-responsive">
            <table class="align-middle text-truncate mb-0 table table-borderless table-hover">
                ...
                <tbody>
                    @foreach ($feature_news as $value)
                        <tr>
                            <td class="text-muted" style="width: 80px;">{{ $loop->iteration }}</td>
                            <td><a href="{{ route('users.show', $value->id) }}">{{ Str::limit($value->title, 20) }}</a></td>
                            <td><a href="{{ route('users.show', $value->id) }}">{{ Str::limit($value->description ?: 'Null', 50) }}</a></td>
                                    ...
                                    <div id="popover-content-{{ $value->id }}" class="d-none">
                                        <div class="dropdown-menu-header">
                                            <div class="dropdown-menu-header-inner bg-success">
                                                ...
                                                <div class="menu-header-content">
                                                    <div class="font-icon-wrapper font-icon-sm"><i class="lnr-user icon-gradient bg-ripe-malin"> </i></div>
                                        ...
                                        <ul class="nav flex-column">
                                            <li class="nav-item"><a href="javascript:void(0);"
                                                    class="nav-link">Created By: {{ $value->created_user->name }} <br>Created At: {{ $value->created_at }}</a></li>
                                            <li class="nav-item-divider nav-item"></li>
                                        </ul>
                                        <ul class="nav flex-column">
                                            <li class="nav-item"><a href="javascript:void(0);"
                                                    class="nav-link">Updated By: {{ $value->updated_user->name }} <br>Updated At: {{ $value->updated_at }}</a></li>
                                        </ul>
                                    </div>

                                        <a href="{{ route('latest-news.edit', $value->id) }}" class="btn-shadow btn btn-primary mr-1"><i class="fa fa-fw"
                                                aria-hidden="true" title="Copy to use edit"></i></a>
        ...
        {{-- Rounded Paginations --}}
        @include('partials._paginate',['style' => 'rounded', 'data' => $feature_news,])
    </div>
@endsection
